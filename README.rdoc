...
`[dir=rtl]` is also used by RTLize as the default `rtl_selector` for matching RTL content. If you prefer to use a different `rtl_selector` (The class `.rtl` for example), then you'll need to make sure your right-to-left HTML matches that selector in order for the RTLize to work properly.
...
When writing your CSS, you will often encounter cases where you need to manually override the CSS for RTL layouts, to change a background image for example. To do this, you can use the `rtl_selector`. RTLize wont transform any CSS rules that use the `rtl_selector`.
...
For example:
  ...
  [dir=rtl] .class-2
    margin-right: 2px; // This will not be transformed

  // This will also not be transformed.
  // Be careful with mixing the `rtl_selector` with other selectors in the same ruleset.
  [dir=rtl] .class-3, .another-class
    float: left;
...
* Add a CLI for transforming CSS files.
...
* This project was inspired by {Dustin Diaz's R2}[http://github.com/ded/R2]. In fact, initially RTLize was not much more than a ruby port of R2.
